...
대상 경로는 반드시 실제 앱의 경로로 확인합니다. 설치 스크립트는 기존
`*.mo`를 백업하지 않고 삭제한 뒤 새 MO로 동기화합니다. 앱 업데이트로 원본 파일이 바뀔 수 있으므로
...
The helper removes existing `*.mo` files and synchronizes the complete MO set
without creating a backup. Other file types are preserved. If a translation problem appears, rebuild from
