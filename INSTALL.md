...
기존 MO를 새 파일로 덮어씁니다. 문제가 생기면 `po/<버전>/ko/`의 원본
PO 또는 `work/<버전>/ko/`의 작업 PO에서 MO를 다시 생성합니다. 빌드가 끝나면
...
대상 경로는 반드시 실제 앱의 경로로 확인합니다. 설치 스크립트는 기존 MO를
백업하지 않고 덮어씁니다. 앱 업데이트로 원본 파일이 바뀔 수 있으므로
게임 버전에 맞는 앱 경로인지 확인하고, 복사 후 게임을 완전히 종료했다가
다시 실행합니다. 앱 패키지가 쓰기
...
기본적으로 언어 선택 창에 `한국어 (<작업버전>-<작업날짜>)` 형식으로
...
프로젝트의 배치 스크립트는 기존 `*.mo`를 별도 백업하지 않고 새 MO로
덮어씁니다. 문제가 생기면 아래 복구 절차를 사용합니다.
...
스크립트는 기존 MO를 백업하지 않고 덮어씁니다. `Program Files` 아래에
설치되어 있으면 관리자 권한이 필요할 수 있습니다.

압축판의 언어 목록에 작업 버전을 표시하려면 `data\languages\ko_KR.cfg`도
다음처럼 수정합니다. 이 스크립트는 `PO_LAST_MODIFIED_DATE`를 사용해
`한국어 (<작업버전>-<작업날짜>)`을 기록하고 원본 설정 파일을 백업합니다.
...
`한국어 (1.18.x-<작업날짜>)`를 선택합니다. 기존 표시로
되돌리려면 같은 디렉터리에 생성된
...
The helper overwrites existing MO files without creating a backup. If a
translation problem appears, rebuild from
`po/<version>/ko/` or `work/<version>/ko/` with `tools/build_mo.sh`, then
install the regenerated files again. `%The Battle for Wesnoth%` means the
`The Battle for Wesnoth.app` bundle under `/Applications` when using the
default install location. App updates may replace the files.
...
By default, the language list shows `한국어 (<작업버전>-<작업날짜>)`. When
...
`한국어 (<work-version>-<work-date>)`, using `PO_LAST_MODIFIED_DATE`.
