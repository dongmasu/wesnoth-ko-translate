...
반드시 별도 디렉터리에 백업합니다. 빌드가 끝나면 `dist/<버전>/ko/MO_BUILD_DATE`
에 MO 생성 날짜(`YYYYMMDD`)가 기록됩니다. 언어 목록의 작업 표식은 이
파일을 우선 사용하므로 `한국어 (<작업버전>-<MO생성일>)` 형식으로 실제
배포 산출물의 세대를 식별할 수 있습니다. 재현 가능한 날짜가 필요하면
`WESNOTH_BUILD_DATE=YYYYMMDD tools/build_mo.sh`처럼 지정합니다.
...
standalone 앱을 별도 복사본으로 실행하면서 앱 번들 내부의 MO를 사용하려면
다음 스크립트를 사용할 수 있습니다. 원본 앱은 유지하고, 복사본의 기존
서명을 제거한 뒤 MO를 설치하고 로컬 ad-hoc 서명을 다시 생성합니다.
...
이 프로젝트에서 실제로 확인한 1.18.x standalone 실행 명령도
다음과 같습니다. 앱을 완전히 종료한 뒤 실행해야 하며, macOS가 다른 앱의
데이터 접근을 물으면 허용합니다.
...
기본적으로 언어 선택 창에 `한국어 (<작업버전>-<MO생성일>)` 형식으로
표시됩니다. `MO_BUILD_DATE`가 있으면 실행일이 아니라 마지막 MO 생성일을
사용합니다. 특정 표식을 사용하려면 두 번째 인자로 지정할 수 있습니다.
...
기존 `*.mo` 파일을 별도 폴더에 백업한 뒤 새 MO를 복사해도 되지만,
프로젝트의 배치 스크립트를 사용하면 자동으로 백업하고 복사할 수 있습니다.
...
다음처럼 수정합니다. 이 스크립트는 `MO_BUILD_DATE`를 사용해
`한국어 (<작업버전>-<MO생성일>)`을 기록하고 원본 설정 파일을 백업합니다.
...
`한국어 (1.18.x-<MO생성일>)`를 선택합니다. 기존 표시로
되돌리려면 같은 디렉터리에 생성된
...
2. 메인 화면 우측 하단의 **언어 설정**을 선택한 뒤 **언어 선택**을
   선택합니다.
3. 언어 선택 창의 좌측 하단에서 **Show in-progress or abandoned
   translations**를 체크합니다.
4. 목록에서 **한국어 (1.18.x-<MO생성일>)**를 선택하고
   게임을 재시작합니다.
5. 번역이 보이지 않으면 게임 버전이 1.18.x인지, MO 파일명이
   `wesnoth*.mo` textdomain 이름과 일치하는지 확인합니다.
6. 기존 MO를 백업했다면 원래 파일을 복원해 변경 전 상태로 되돌릴 수
   있습니다.
7. 여러 Wesnoth 설치본이 있으면 실행 중인 게임의 데이터 경로에 복사했는지
   확인합니다.
...
overwriting them. Each build records its actual generation date as
`dist/<version>/ko/MO_BUILD_DATE` in `YYYYMMDD` form. The language-list marker
uses this date, producing `한국어 (<work-version>-<MO-build-date>)`. Set
`WESNOTH_BUILD_DATE=YYYYMMDD` when a reproducible build date is required.
...
#### Avoid modifying the macOS app bundle

macOS applications contain signed resources. Replacing MO files inside either
an App Store or standalone bundle can produce “application is damaged” or
`sealed resource is missing or invalid`. Do not modify the bundle. Use
Wesnoth's `--data-dir` option with a writable external data directory:
...
By default, the language list shows `한국어 (<작업버전>-<MO생성일>)`. When
`MO_BUILD_DATE` exists, the helper uses the latest MO build date rather than
the script execution date. Pass a second argument to use a specific marker.
The helper
...
To run a standalone app copy with bundled MO files, use the following helper.
It preserves the original app, removes the copy's distributor signature,
installs the MO files, and applies a local ad-hoc signature:
...
The first helper backs up existing MO files before copying. The second helper
backs up `ko_KR.cfg` and writes the language label as
`한국어 (<work-version>-<MO-build-date>)`, using `MO_BUILD_DATE`.
...
6. Restore the backed-up MO files to undo the change.
7. If multiple Wesnoth copies are installed, verify that the files were
