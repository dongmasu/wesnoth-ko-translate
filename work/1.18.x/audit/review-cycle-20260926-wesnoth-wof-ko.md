...
- 가알 섬 도입부에서 `영지 중`, `어쩔 수 없이`와 같은 띄어쓰기와
  먹잇감의 주체를 자연스럽게 정리했다.
- 원문 의미를 반대로 만든 `버거운 것이 아닐 거예요`를 `버거울 거예요`로
  수정했다.
- 카르론과 고를락 사이의 대화에서 불필요한 `당신`을 `그대`로 조정하고,
  `먹잇감 표식`, `합류시키지`, `바보 같은` 등의 표현을 정리했다.
- Fire Guardian의 낮은 속삭임을 `너무 작아서`로 오역한 문장을
  `너무 낮은 목소리로`로 수정했다.
- 고유 유닛 설명의 `Wyrm`을 확정 예외 규칙에 따라
  `웜(Wyrm)`으로 표기했다. 멸칭 문맥의 `도마뱀`은 유지했다.
